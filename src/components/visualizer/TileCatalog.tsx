...
export default function TileCatalog({
  ...
  return (
    <aside className="flex min-h-0 flex-col rounded-2xl border border-mist bg-white shadow-sm">
      <div className="border-b border-mist p-4">
        <div className="flex items-center justify-between gap-3">
          <h2 className="font-display text-sm font-bold text-navy">Tile Library</h2>
          <button
            type="button"
        ...
        <div className="relative mt-3">
          ...
          <input
            ...
            className="w-full rounded-lg border border-mist bg-off py-2 pl-9 pr-3 text-sm text-navy placeholder:text-muted/60 outline-none transition-colors focus:border-green/60"
          />
        </div>
          ...
          {tileCategories.map((c) => (
            <button
              ...
              className={`rounded-full px-2.5 py-1 text-[0.68rem] font-semibold transition-colors ${
                category === c.key
                  ? "bg-green text-white"
                  : "bg-navy/5 text-muted hover:bg-navy/10 hover:text-navy"
              }`}
            >
              {c.label}
            </button>
      ...
      >
        {list.length === 0 ? (
          <div className="px-4 py-14 text-center">
            <div className="flex justify-center text-muted/50" aria-hidden="true">
                  ...
                  <div
                    className={`group relative flex w-full items-center gap-3 rounded-xl border p-2 transition-all ${
                      selected
                        ...
                        : "border-mist bg-off hover:border-navy/25 hover:bg-white"
                    }`}
                  >
                    <button
                      type="button"
                    ...
                    <button
                      ...
                      aria-pressed={fav}
                      className="shrink-0 rounded-full p-2 text-muted/60 transition-colors hover:bg-navy/5 hover:text-navy"
                    >
                      <Heart filled={fav} />
                    </button>
                    {selected && (
      ...
      <div className="border-t border-mist px-4 py-2.5 text-[0.68rem] text-muted/80">
